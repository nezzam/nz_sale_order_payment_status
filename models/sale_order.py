...
class SaleOrder(models.Model):
    ...
    @api.depends("invoice_ids.state", "invoice_ids.amount_total", "invoice_ids.amount_residual", "amount_total",)
    def _compute_payment_info(self):
        for order in self:
            total_paid = 0.0
            order_total = order.amount_total or 0.0
            posted_invoices = order.invoice_ids.filtered(lambda i: i.state == "posted" and i.move_type == "out_invoice")
    ...
    def _compute_payment_progress(self):
        for order in self:
            ...
            if total:
                ...
                # order.payment_progress = min((order.paid_amount / order.amount_total) * 100, 100)
            else:
